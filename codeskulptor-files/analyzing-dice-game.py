# To run it use this url : https://py2.codeskulptor.org/#user48_HljmtBDYn0_0.py
...
def compute_expected_value():
    ...
    for seq in all_rolls:
        if max_repeats(seq) == 2:
            list_wins.append(2)
        elif max_repeats(seq) == 3:
            list_wins.append(3)
    num_2s = list_wins.count(2)
    num_3s = list_wins.count(3)
    expected_value = (float(num_2s) / len(all_rolls)) * 10 + (float(num_3s) / len(all_rolls)) * 200
